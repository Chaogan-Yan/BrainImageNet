...
# Predicting sex by Brain Image (preprocessed gray matter density/volume data) 
...
# FORMAT python3 y_Predict_Sex.py -i /in -o /out
...
# /out -- the output dir. There would be Sex_Prediction.txt after prediction.
...
def Sex_Classifier(InputDir, OutputDir):
    ...
    file_write_obj = open(OutputDir+'/Sex_Prediction.txt', 'w')
    for i in range(len(FullList)):
        file_write_obj.write(SubList[FullList[i]]+':\t'+str(Prediction[i])+' \n')
    file_write_obj.close()
    file_write_obj = open(OutputDir+'/Sex_Prediction.txt', 'a')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Predict Sex')
    ...
    Sex_Classifier(InputDir=args.i, OutputDir=args.o)
